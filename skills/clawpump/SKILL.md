...
- **Which balance pays?** x402 / pay.sh / normal payments spend the agent's **public on-chain USDC** — check it with `get_wallet_summaries` (`usdc_balance`). **Do NOT** use `get_private_wallet_balance` to gauge payment funds: that is the **MagicBlock private balance** (for private transfers / ephemeral rollup) and is unrelated to x402 — it's usually 0 and will make you wrongly think the agent is out of funds. Only touch `get_private_wallet_balance` when the user explicitly asks about private/MagicBlock balances.
