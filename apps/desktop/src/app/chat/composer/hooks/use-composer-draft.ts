...
import { $composerAttachments, type ComposerAttachment, $pendingChatPrompt, stashSessionDraft, takeSessionDraft } from '@/store/composer'
...
export function useComposerDraft({
  ...
  useEffect(() => {
    ...
    // One-shot prompt from a "use in chat" flow (x402 / Wallet / Agent Mail)
    // that navigated to a fresh chat. Drain it AFTER the session-draft load so
    // it isn't overwritten; append if there's already a draft. (ClawPump)
