...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      retryingRemoteBackend: '正在重新連線遠端 Claw Agent 後端…',
  ...
  settings: {
    ...
    notifications: {
      ...
      kinds: {
        ...
        plugin: {
          label: '外掛通知',
          description: 'Claw Agent 在背景時，桌面外掛傳送了通知。'
        }
      },
      test: '傳送測試通知',
    ...
    appearance: {
      ...
      reactionsTitle: '訊息回應',
      reactionsDesc: 'iMessage 風格的表情回應 — 你可以對訊息做出回應，Claw Agent 也能回應你的訊息。',
      ...
      pet: {
        title: '寵物',
        intro:
          '領養一隻懸浮在應用上的 petdex 動畫寵物，它會根據 Claw Agent 的狀態做出反應——工具執行時奔跑、成功時歡呼、出錯時沮喪。',
        restartHint: '寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 Claw Agent，然後回到此處。',
        scaleTitle: '大小',
    ...
    about: {
      ...
      bundleOutOfSyncDesc:
        'Claw Agent 執行環境已更新,但桌面應用程式本身仍是舊建置——在應用程式更新之前,新的介面功能(如 Bot Mode)不會顯示。請執行下方的更新以重新建置應用程式。如果此警告仍未消除,請從最新的桌面安裝程式重新安裝。',
    ...
    quickEntry: {
      enabledTitle: '快速輸入',
      enabledDesc: '用全域快速鍵在任何地方喚出一個小輸入框，無需開啟 Claw Agent 即可送出提示。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Claw Agent Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Claw Agent 後端，請使用遠端閘道。閘道連線屬於本機層級設定；設定檔是從已連線的閘道中探索出來的。',
      ...
      sshTitle: '透過 SSH 連線',
      sshDesc:
        'Claw Agent 會透過 SSH 在遠端啟動並以通道連線到本應用程式——無需自行啟動或公開任何服務。前提：已具備到該主機的金鑰 SSH 存取。',
      ...
      sshHermesPathTitle: 'Claw Agent 路徑（選用）',
      ...
      sshReachable: (host, platform) => `可連線：${host}（${platform}）——已找到 Claw Agent`,
      sshIncompleteHost: '連線前請輸入 SSH 主機。',
      sshErrUnreachable: '無法透過 SSH 連線到該主機。請檢查主機、連接埠和網路。',
      sshErrAuth:
        'SSH 驗證失敗。請將金鑰載入 ssh-agent（ssh-add），或在 ~/.ssh/config 中設定 IdentityFile——Claw Agent 以非互動方式執行 ssh。',
      sshErrHostKey: '自上次連線以來主機金鑰已變更。請確認這是預期的，然後執行 ssh-keygen -R <host> 並重新連線。',
      sshErrNotInstalled:
        '遠端主機上未安裝 Claw Agent。請在遠端安裝（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）或設定 Claw Agent 路徑。',
      sshErrPlatform: '不支援的遠端平台。Claw Agent Desktop 的 SSH 模式支援 Linux、macOS 和 Windows 遠端主機。',
      sshErrTimeout: 'SSH 連線逾時。主機可能無法存取或處於睡眠狀態。',
      sshErrUpdateRequired: '使用 Desktop SSH 連線前，請更新遠端主機上的 Claw Agent。',
    ...
    mcp: {
      ...
      deepLinkDescription: '一個連結要求將此 MCP 伺服器加入 Claw Agent。請檢查下方的完整設定——它來自該連結，而非 Claw Agent。',
    ...
    providers: {
      ...
      localEndpoint: {
        title: '本地 / 自訂端點',
        description: '將 Claw Agent 指向任意 OpenAI 相容端點（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在載入提供方...'
    },
    sessions: {
  ...
  starmap: {
    ...
    loading: '載入中…',
    emptyTitle: '尚無學習內容',
    emptyDesc: '當 Claw Agent 為你的工作建立技能與記憶時，會顯示在這裡。'
  },
  agents: {
    close: '關閉代理',
    title: '派生樹',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '請重新啟動 Claw Agent 以使用寵物功能。',
      empty: '沒有符合的寵物。',
      turnOff: '關閉',
      turnOn: '開啟',
      installed: '已安裝',
    ...
    generatePet: {
      ...
      staleBackend: '請更新 Claw Agent 以生成寵物。',
      backgroundHint: '你可以關閉此視窗——完成後 Claw Agent 會通知你。',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      review: '檢查排程工作',
      saveFailed: 'Claw Agent 未儲存該模型變更。'
    },
    search: '搜尋排程工作…',
    loading: '正在載入排程工作…',
    states: {
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '無法建立專案',
      staleBackend: '請更新 Claw Agent 後端以建立專案——目前後端比桌面應用舊（設定 → 更新 → 後端）。',
      deleteConfirm: '這會從 Claw Agent 中移除已儲存的專案。檔案、git 儲存庫和工作樹維持不變。',
      startWork: '新增工作樹',
      ...
      worktreeStaleBackend: '請更新 Claw Agent 後端以在此遠端連線上建立工作樹 —— 該後端早於 git 工作樹 API。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: '讓 Claw Agent 提交並開 PR',
  ...
  updates: {
    ...
    guiSkewBody:
      '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 Claw Agent 桌面應用程式（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody:
      'Claw Agent 更新程式會在自己的視窗中接管，並在完成後自動重新開啟 Claw Agent。更新期間請勿自行重新開啟 Claw Agent。',
    applyingBodyBackend: '遠端後端正在套用更新並將重新啟動。恢復後 Claw Agent 會自動重新連線。',
    applyingClose: '此視窗會在更新期間關閉，隨後 Claw Agent 會自動重新開啟。',
    errorTitle: '更新未完成',
    errorBody: '沒有資料遺失。您可以現在重試。',
    blockerTitle: '關閉本機預覽以更新 Claw Agent？',
    blockerBody: 'Claw Agent 需要在更新前停止這些本機預覽。這不會修改或刪除您的檔案。',
    foreignBlockerTitle: '關閉其他處理程序以更新 Claw Agent',
    foreignBlockerBody:
      'Claw Agent 無法安全地自動關閉這些處理程序。請關閉擁有這些處理程序的應用程式、終端機或服務，然後重試更新。',
    mixedBlockerBody: 'Claw Agent 可以關閉下方列出的本機預覽。其他處理程序必須手動關閉，更新才能繼續。',
  ...
  install: {
    ...
    setupChoiceTitle: '設定 Claw Agent Desktop',
    setupChoiceDesc: '將此應用程式連線到您已執行的 Claw Agent 閘道，或在這台電腦上本機安裝 Claw Agent。',
    connectExistingTitle: '連線到現有 Claw Agent',
    connectExistingShort: '連線現有環境',
    connectExistingDesc: '使用工作階段權杖或瀏覽器登入連線遠端後端。不會啟動本機安裝。',
    installLocalTitle: '本機安裝 Claw Agent',
    installLocalDesc: '下載 Claw Agent、建立 Python 環境，並在這台電腦上執行後端。',
    localStartUnavailable: '無法啟動本機安裝。請重新啟動 Claw Agent Desktop 後再試一次。',
    remoteSetupTitle: '連線到現有 Claw Agent',
    remoteSetupDesc: '輸入閘道 URL。Claw Agent Desktop 會偵測需要權杖還是瀏覽器登入。',
    remoteUrlTitle: '閘道 URL',
    remoteUrlDesc: '使用 Claw Agent 閘道的基礎 URL；遠端位址請包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: '正在偵測閘道驗證方式...',
    probeError: '無法連線到該 Claw Agent 閘道。',
    ...
    backToSetup: '返回',
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 Claw Agent Agent',
    finishingTitle: '正在收尾',
    failedDesc:
  ...
  onboarding: {
    headerTitle: '開始設定 Claw Agent Agent',
  ...
  preview: {
    ...
    web: {
      ...
      blankPageBody: '在上方輸入網址開始瀏覽，或請 Claw Agent 開啟頁面。',
