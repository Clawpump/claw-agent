...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      retryingRemoteBackend: '正在重新连接远程 Claw Agent 后端…',
  ...
  settings: {
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 Claw Agent 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 Claw Agent 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
        ...
        updateBackendToManage: '请更新 Claw Agent 后端以便在桌面端管理此插件。',
    ...
    notifications: {
      ...
      kinds: {
        ...
        plugin: {
          label: '插件通知',
          description: 'Claw Agent 在后台时，桌面插件发送了通知。'
        }
      },
      test: '发送测试通知',
    ...
    appearance: {
      ...
      reactionsTitle: '消息回应',
      reactionsDesc: 'iMessage 风格的表情回应 — 你可以给消息添加回应，Claw Agent 也能回应你的消息。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 Claw Agent 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 Claw Agent，然后回到此处。',
        scaleTitle: '大小',
    ...
    about: {
      ...
      bundleOutOfSyncDesc:
        'Claw Agent 运行时已更新,但桌面应用本身仍是旧构建——在应用更新之前,新的界面功能(如 Bot Mode)不会显示。请运行下方的更新以重新构建应用。如果此警告仍未消除,请从最新的桌面安装程序重新安装。',
    ...
    quickEntry: {
      enabledTitle: '快速输入',
      enabledDesc: '用全局快捷键在任何地方唤出一个小输入框，无需打开 Claw Agent 即可发送提示。',
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及通过远程、SSH 或 Hermes Cloud 连接可访问的每个 Claw Agent 网关。',
      stagedNote:
      ...
      kindSsh: 'SSH',
      kindLocalDesc: '由本应用管理的 Claw Agent 运行时。',
      kindRemoteDesc: '可通过 HTTP(S) 访问的 Claw Agent 网关——局域网、Tailscale 或互联网。',
      kindCloudDesc: '通过你的 Hermes Cloud 账户发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 Claw Agent 安装。',
      labelTitle: '名称',
      ...
      headersDesc:
        '随发往此网关的每个 HTTP 和 WebSocket 请求一起发送——用于 Cloudflare Access 等访问代理（CF-Access-Client-Id / CF-Access-Client-Secret）。值加密存储。由 Claw Agent 管理的请求头（Authorization、Cookie、Host 等）会被忽略。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Claw Agent Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Claw Agent 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
      ...
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 Claw Agent 后端。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Claw Agent Desktop 将使用已保存设置重新连接（界面保持打开）。',
      ...
      sshTitle: '通过 SSH 连接',
      sshDesc:
        'Claw Agent 会通过 SSH 在远程启动并以隧道连接到本应用——无需自行启动或暴露任何服务。前提：已具备到该主机的密钥 SSH 访问。',
      ...
      sshHermesPathTitle: 'Claw Agent 路径（可选）',
      ...
      sshReachable: (host, platform) => `可连接：${host}（${platform}）——已找到 Claw Agent`,
      sshIncompleteHost: '连接前请输入 SSH 主机。',
      sshErrUnreachable: '无法通过 SSH 连接到该主机。请检查主机、端口和网络。',
      sshErrAuth:
        'SSH 认证失败。请将密钥加载到 ssh-agent（ssh-add），或在 ~/.ssh/config 中设置 IdentityFile——Claw Agent 以非交互方式运行 ssh。',
      sshErrHostKey: '自上次连接以来主机密钥已更改。请确认这是预期的，然后运行 ssh-keygen -R <host> 并重新连接。',
      sshErrNotInstalled:
        '远程主机上未安装 Claw Agent。请在远程安装（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）或设置 Claw Agent 路径。',
      sshErrPlatform: '不支持的远程平台。Claw Agent Desktop 的 SSH 模式支持 Linux、macOS 和 Windows 远程主机。',
      sshErrTimeout: 'SSH 连接超时。主机可能无法访问或处于休眠状态。',
      sshErrUpdateRequired: '使用 Desktop SSH 连接前，请更新远程主机上的 Claw Agent。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        '一个链接请求将此 MCP 服务器添加到 Claw Agent。请检查下方的完整配置——它来自该链接，而非 Claw Agent。',
    ...
    providers: {
      ...
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 Claw Agent 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在加载提供方...'
    },
    sessions: {
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 Claw Agent 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 Claw Agent 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 Claw Agent 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 Claw Agent 会通知你。',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      review: '检查定时任务',
      saveFailed: 'Claw Agent 未保存该模型更改。'
    },
    search: '搜索定时任务…',
    loading: '正在加载定时任务…',
    states: {
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 Claw Agent 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 Claw Agent 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
      ...
      worktreeStaleBackend: '请更新 Claw Agent 后端以在此远程连接上创建工作树 —— 该后端早于 git 工作树 API。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: '让 Claw Agent 提交并开 PR',
  ...
  updates: {
    ...
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 Claw Agent 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      'Claw Agent 更新器会在自己的窗口中接管，并在完成后自动重新打开 Claw Agent。更新期间请不要自行重新打开 Claw Agent。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Claw Agent 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 Claw Agent 会自动重新打开。',
    errorTitle: '更新未完成',
    errorBody: '没有数据丢失。你可以现在重试。',
    blockerTitle: '关闭本地预览以更新 Claw Agent？',
    blockerBody: 'Claw Agent 需要在更新前停止这些本地预览。这不会修改或删除你的文件。',
    foreignBlockerTitle: '关闭其他进程以更新 Claw Agent',
    foreignBlockerBody: 'Claw Agent 无法安全地自动关闭这些进程。请关闭拥有这些进程的应用、终端或服务，然后重试更新。',
    mixedBlockerBody: 'Claw Agent 可以关闭下方列出的本地预览。其他进程必须手动关闭，更新才能继续。',
  ...
  install: {
    ...
    setupChoiceTitle: '设置 Claw Agent Desktop',
    setupChoiceDesc: '将此应用连接到你已运行的 Claw Agent 网关，或在这台电脑上本地安装 Claw Agent。',
    connectExistingTitle: '连接到现有 Claw Agent',
    connectExistingShort: '连接现有环境',
    connectExistingDesc: '使用会话令牌或浏览器登录连接远程后端。不会启动本地安装。',
    installLocalTitle: '本地安装 Claw Agent',
    installLocalDesc: '下载 Claw Agent，创建 Python 环境，并在这台电脑上运行后端。',
    localStartUnavailable: '无法启动本地安装。请重启 Claw Agent Desktop 后重试。',
    remoteSetupTitle: '连接到现有 Claw Agent',
    remoteSetupDesc: '输入网关 URL。Claw Agent Desktop 会检测需要令牌还是浏览器登录。',
    remoteUrlTitle: '网关 URL',
    remoteUrlDesc: '使用 Claw Agent 网关的基础 URL；远程地址请包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: '正在检测网关认证方式...',
    probeError: '无法连接到该 Claw Agent 网关。',
    identityProvider: '你的身份提供方',
    authTitle: '认证',
    ...
    backToSetup: '返回',
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Claw Agent Agent',
    finishingTitle: '正在收尾',
    failedDesc:
  ...
  onboarding: {
    headerTitle: '开始设置 Claw Agent Agent',
  ...
  preview: {
    ...
    web: {
      ...
      blankPageBody: '在上方输入地址开始浏览，或让 Claw Agent 打开一个页面。',
