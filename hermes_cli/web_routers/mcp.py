...
async def list_mcp_servers(profile: Optional[str] = None):
    ...
    def _read():
        with _profile_scope(profile):
            ...
            # Authentication checks read the selected profile's .env/OAuth
            # state, so summaries must be built before leaving this scope.
            return [
                _mcp_server_summary(name, cfg)
                for name, cfg in sorted(servers.items())
            ]

    return {"servers": await asyncio.to_thread(_read)}
