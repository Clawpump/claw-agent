...
# Configuration
REPO_URL_SSH="git@github.com:Clawpump/claw-agent.git"
REPO_URL_HTTPS="https://github.com/Clawpump/claw-agent.git"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/Clawpump/claw-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            case "$current_origin" in
                *[Nn]ous[Rr]esearch/hermes-agent*)
...
install_desktop() {
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Claw Agent" ]; then
            app="$desktop_dir/release/linux-unpacked/Claw Agent"
